...
// ─── 마이그레이션: 박스/판 단위 → 개 단위 (표시 라벨만 변경, 수량은 그대로 유지) ───
// v136: 박스→개 (1회), v137: 판→개 (1회, 박스 마이그레이션과 별개 플래그)
(function migrateStockUnitBoxToPiece() {
    if (localStorage.getItem('stockUnitMigV136')) return;
    let changed = false;
    stockItems.forEach(si => {
        if (si && typeof si.unit === 'string' && si.unit.trim() === '박스') { si.unit = '개'; changed = true; }
    });
...
(function migrateStockUnitTrayToPiece() {
    if (localStorage.getItem('stockUnitMigV137')) return;
    let changed = false;
    stockItems.forEach(si => {
        if (si && typeof si.unit === 'string' && si.unit.trim() === '판') { si.unit = '개'; changed = true; }
    });
    localStorage.setItem('stockUnitMigV137', '1');
    if (changed) {
        saveData();
        if (typeof _markDirty === 'function') _markDirty('stock');
    }
